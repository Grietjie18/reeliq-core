...
async def get_fuel_prices():
    ...
    try:
        ...
        try:
            async with httpx.AsyncClient(timeout=8) as client:
                r = await client.get("https://www.aa.co.za/fuel-price-history/")
            text = r.text
            petrol_match = re.search(r'95\s*ULP[^\d]+([\d]+\.[\d]+)', text)
...
async def plan_route(
    ...
    LATERAL_OFFSETS = [-0.04, -0.02, 0.0, 0.02, 0.04]
    ...
    optimised_fuel_l = min(optimised_fuel_l, direct_fuel_l)
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .login-field input:focus {
            border-color: var(--cyan); background: rgba(0,242,255,0.08);
        }
        .login-btn {
        ...
        #route-btn {
            ...
            background: rgba(0,242,255,0.08); border: 1px solid var(--cyan-border);
            border-radius: 2px; color: var(--cyan);
            font-family: 'Share Tech Mono', monospace; font-size: 0.65rem;
            letter-spacing: 0.1em; padding: 8px 14px; cursor: pointer; transition: background 0.2s;
        }
        #route-btn:hover { background: var(--cyan-dim); }
        #route-btn.active { background: var(--cyan); color: var(--bg); }
        #route-panel {
            display: none; position: absolute; bottom: 110px; left: 16px; z-index: 1002;
            background: var(--panel); border: 1px solid var(--cyan-border);
            border-radius: 4px; padding: 16px; width: 240px; backdrop-filter: blur(12px);
        }
        #route-panel .rp-title { font-size: 0.65rem; letter-spacing: 0.2em; color: var(--cyan); text-transform: uppercase; margin-bottom: 12px; }
        #route-panel label { font-size: 0.6rem; letter-spacing: 0.15em; color: rgba(0,242,255,0.5); text-transform: uppercase; display: block; margin-bottom: 4px; margin-top: 10px; }
        #route-panel input, #route-panel select {
            width: 100%; background: rgba(0,242,255,0.04); border: 1px solid var(--cyan-border);
            border-radius: 2px; padding: 8px 10px; color: var(--cyan);
            font-family: 'Share Tech Mono', monospace; font-size: 0.8rem; outline: none;
        }
        #route-panel select option { background: #04080f; }
        .rp-step {
            font-size: 0.6rem; color: rgba(0,242,255,0.6); letter-spacing: 0.1em; margin-top: 10px;
            padding: 8px; border: 1px dashed rgba(0,242,255,0.2); border-radius: 2px; text-align: center;
        }
        .rp-step.done { border-color: var(--good); color: var(--good); }
        .rp-btn {
            width: 100%; margin-top: 12px; padding: 10px; background: transparent;
            border: 1px solid var(--cyan); border-radius: 2px; color: var(--cyan);
            font-family: 'Share Tech Mono', monospace; font-size: 0.65rem;
            letter-spacing: 0.15em; cursor: pointer; text-transform: uppercase; transition: background 0.2s;
        }
        .rp-btn:hover { background: var(--cyan-dim); }
        .rp-btn:disabled { opacity: 0.3; cursor: not-allowed; }
        .rp-clear {
            width: 100%; margin-top: 6px; padding: 6px; background: transparent;
            border: 1px solid rgba(255,59,59,0.3); border-radius: 2px; color: var(--bad);
            font-family: 'Share Tech Mono', monospace; font-size: 0.6rem;
            letter-spacing: 0.1em; cursor: pointer; text-transform: uppercase;
        }
        #route-result {
            display: none; position: absolute; top: 50%; left: 50%;
            transform: translate(-50%, -50%); z-index: 1010;
            background: var(--panel); border: 1px solid var(--cyan-border);
            border-radius: 6px; padding: 24px 28px; width: min(360px, 92vw);
            backdrop-filter: blur(20px); max-height: 90vh; overflow-y: auto;
        }
        #route-result .rr-title {
            font-family: 'Orbitron', sans-serif; font-size: 0.75rem;
            letter-spacing: 0.2em; color: var(--cyan); margin-bottom: 16px; text-transform: uppercase;
        }
        .rr-row {
            display: flex; justify-content: space-between; align-items: center;
            padding: 8px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-size: 0.7rem;
        }
        ...
        .rr-disclaimer {
            margin-top: 14px; padding: 10px; border: 1px solid rgba(255,184,0,0.2);
            border-radius: 2px; font-size: 0.55rem; color: rgba(255,184,0,0.6); line-height: 1.5; letter-spacing: 0.05em;
        }
        .rr-close {
            width: 100%; margin-top: 14px; padding: 10px; background: transparent;
            border: 1px solid rgba(255,255,255,0.15); border-radius: 2px; color: rgba(255,255,255,0.4);
        ...
        .dm-box {
            background: var(--panel); border: 1px solid var(--warn); border-radius: 6px;
            padding: 32px; width: min(380px, 92vw); backdrop-filter: blur(20px);
        }
        .dm-title { font-family: 'Orbitron', sans-serif; color: var(--warn); font-size: 0.8rem; letter-spacing: 0.15em; margin-bottom: 16px; }
        .dm-text { font-size: 0.7rem; line-height: 1.7; color: rgba(255,255,255,0.6); margin-bottom: 20px; }
        .dm-btn {
            width: 100%; padding: 12px; background: transparent; border: 1px solid var(--warn);
            border-radius: 2px; color: var(--warn); font-family: 'Share Tech Mono', monospace;
            font-size: 0.7rem; letter-spacing: 0.15em; cursor: pointer; text-transform: uppercase; transition: background 0.2s;
        }
...
<div id="app-screen">
    ...
    <div id="route-panel">
        ...
        <div style="font-size:0.55rem;color:rgba(0,242,255,0.35);margin-top:4px;">Saved per vessel — edit anytime</div>
        ...
        <button class="rp-btn" id="rp-calculate-btn" onclick="calculateRoute()" disabled>Calculate Route</button>
...
L.CanvasHeatOverlay = L.Layer.extend({
    ...
    _show() { if (this._canvas) this._canvas.style.opacity = '1'; },
    _redraw() {
...
function handleMapClick(e) {
    ...
    if (routeMode === 'start') {
        ...
        const m = L.circleMarker([lat, lng], {
            radius: 7, fillColor: '#00ff88', color: '#fff', weight: 2, fillOpacity: 1
        }).bindTooltip('START', {permanent: true}).addTo(map);
        routeMarkers.push(m);
        ...
        routeEnd = [lat, lng];
        const m = L.circleMarker([lat, lng], {
            radius: 7, fillColor: '#ff3b3b', color: '#fff', weight: 2, fillOpacity: 1
        }).bindTooltip('DEST', {permanent: true}).addTo(map);
        routeMarkers.push(m);
...
async function calculateRoute() {
    ...
    if (!consumption || consumption <= 0) { alert("Please enter your boat's fuel consumption (L/hr)"); return; }
    if (!routeStart || !routeEnd) { alert("Please set both start and destination points on the map"); return; }
    saveFuelPrefs();
    ...
    try {
        ...
        routeLayer = L.polyline(data.route, { color: '#00f2ff', weight: 3, opacity: 0.85, dashArray: '8 4' }).addTo(map);
        ...
        rows.innerHTML = `
            ...
            <div class="rr-row"><span class="rr-key" style="color:rgba(255,184,0,0.5);font-size:0.6rem;max-width:100%;">${data.best_departure_reason}</span></div>
        `;
...
function closeRouteResult() { document.getElementById('route-result').style.display = 'none'; }
