  ...
  fetch:
    ...
    steps:
      ...
      - name: Install dependencies
        run: pip install copernicusmarine numpy shapely xarray h5py h5netcdf

      - name: Configure Copernicus credentials
        env:
          COPERNICUS_USERNAME: ${{ secrets.COPERNICUS_USERNAME }}
          COPERNICUS_PASSWORD: ${{ secrets.COPERNICUS_PASSWORD }}
        run: |
          copernicusmarine login --username "$COPERNICUS_USERNAME" --password "$COPERNICUS_PASSWORD" --overwrite-configuration-file
